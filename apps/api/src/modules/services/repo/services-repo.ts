...
import type { Service } from '../domain/services-domain.js';
...
function mapServiceRow(row: Record<string, unknown>): Service {
  const priceValue = row.price_amount;
  const price = priceValue === null || priceValue === undefined ? 0 : Number(priceValue);
  return {
    id: row.id as string,
